...
class myResnet(nn.Module):
    def __init__(self,
        ...
        # Different from pytorch implementation. I explicitly show what is the in channel as opposed to as opposed to just using the out_channel or `planes` if using terminology from pytorch implementation`
    ...
    def _make_layer(self,
        ...
        """Create a layer as in resnet paper. In each layer, there are multiple blocks denoted by'blocks'. 
        The first block is the one where we do downsampling and stride, because we know in each later (multiple blocks), the HW is only divided by 2 once.
        For the remaining blocks, we do not need to do downsampling or stride."""
